...
def SendInfoAboutExecution(id):
    ...
    api_url = config_info['DEFAULT']['SERVER']+"/api/v1/execution/"+str(id)+"/status"
    # msg = {"secret":SEGREDO}
    print(api_url)
...
def GetConfig():
    global CONFIG_OF_EXP
    api_url = config_info['DEFAULT']['SERVER']+"/api/v1/apparatus/"+config_info['DEFAULT']['APPARATUS_ID']
    # msg = {"secret":SEGREDO}
    print(api_url)
...
def GetExecution():
    global next_execution
    api_url = config_info['DEFAULT']['SERVER']+"/api/v1/apparatus/"+config_info['DEFAULT']['APPARATUS_ID']+"/nextexecution"
    response =  requests.get(api_url,headers = HEADERS)
...
def SendPartialResult(msg):
    ...
    api_url = config_info['DEFAULT']['SERVER']+"/api/v1/result"
    if config_info['DEFAULT']['DEBUG'] == "on":
        print(str(msg))
        print(api_url)
