...
def receive_data_from_exp(send_config,config):
    ...
    return Cavity.Do_experiment(send_config,config["id"],list_of_ports["pressure_gage"], list_of_ports["arinst"],config["config"]["f_strat"]*10**6, config["config"]["f_end"]*10**6, config["config"]["f_step"]*10**6, config["config"]["n_iteration"], config["config"]["back_pressure"]/100,config["config"]["pressure"],config["config"]["gas_selector"],config["config"]["discharge"],config["config"]["magnite_field"])
