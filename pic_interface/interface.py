...
def receive_data_from_exp(config):
    ...
    return Cavity.Do_experiment(list_of_ports["pressure_gage"], list_of_ports["arinst"],config["id"],config["config"]["f_strat"]*10**6, config["config"]["f_end"]*10**6, config["config"]["f_step"]*10**6, config["config"]["n_iteration"], config["config"]["back_pressure"],config["config"]["pressure"],config["config"]["gas_selector"])
